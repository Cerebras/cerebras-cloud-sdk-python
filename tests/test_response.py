...
from cerebras.cloud.sdk import Cerebras, BaseModel, AsyncCerebras
from cerebras.cloud.sdk._response import (
    APIResponse,
    BaseAPIResponse,
    AsyncAPIResponse,
    BinaryAPIResponse,
...
from cerebras.cloud.sdk._streaming import Stream
from cerebras.cloud.sdk._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'cerebras.cloud.sdk._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: Cerebras) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from cerebras.cloud.sdk import BaseModel`",
    ):
...
def test_response_parse_bool(client: Cerebras, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncCerebras, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,
...
def test_response_parse_expect_model_union_non_json_content(client: Cerebras) -> None:
    response = APIResponse(
...
async def test_async_response_parse_expect_model_union_non_json_content(async_client: AsyncCerebras) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo", headers={"Content-Type": "application/text"}),
        client=async_client,
        stream=False,
